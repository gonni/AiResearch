...
import stock.representation.StockDataSetIterator;
...
public class StockPricePrediction {
    ...
    static int exampleLength = 22; // time series length, assume 22 working days per month

    public static void main (String[] args) throws IOException {
//        String file = new ClassPathResource("prices-split-adjusted.csv").getFile().getAbsolutePath();
        String file = "/Users/ygkim/IdeaProjects/StockPrediction/src/main/resources/prices-split-adjusted.csv";
    ...
    static void predictPriceOneAhead (MultiLayerNetwork net, List<Pair<INDArray, INDArray>> testData,
                                      double max, double min, PriceCategory category) {
        double[] predicts = new double[testData.size()];
        double[] actuals = new double[testData.size()];
        System.out.println("---> Predict TestData Size : " + testData.size());
        for (int i = 0; i < testData.size(); i++) {
            double a = (testData.get(i).getKey()).getDouble(exampleLength - 1);

            predicts[i] = net.rnnTimeStep(testData.get(i).getKey())
                    .getDouble(exampleLength - 1) * (max - min) + min;
...
//            System.out.println(a + "->" + predicts[i] + "/" + actuals[i]);
    ...
    static void predictPriceMultiple (MultiLayerNetwork net,
